  ...
  return (
    ...
    <a href="#" className='text-white flex items-center font-sans font-[500]'>
      ...
      <h1 className = "text-2xl font-bold text-gradient mx-1">Moonshot</h1>
        ...
        <button><a href="#" className='bg-gradient rounded-full px-8 py-3 text-white font-sans font-[700] cursor-pointer sm:flex hidden hover:text-gradient hover:bg-white'>Get the App</a></button>
        <div className='sm:hidden flex items-end justify-end '>
            <RxHamburgerMenu className='text-2xl text-gradient ' onClick={handleToggle} />
            <div className={`${toggle ? "right-0" : "right-[-600px]"} fixed top-0 bg-primary w-[60%] h-[70%] p-6 z-10 sidenav`}>
                <IoMdClose className='text-4xl text-gradient' onClick={handleToggle}  />
                ...
                {links.map((link)=>(
                    <li key={link.id} className='text-white font-sans text-[18px] cursor-pointer mb-10'>
                        <a className='hover:text-gradient' href={`#${link.id}`}>{link.title}</a>
                    </li>
                ))}
                </ul> 
                <div className='flex flex-col items-center justify-center'>
                <button><a href="#" className='bg-gradient rounded-full px-8 py-3 text-white font-sans font-[700] cursor-pointer hover:text-gradient hover:bg-white'>Get the App</a></button>
                </div>
            </div>
            </div>
    </nav>
